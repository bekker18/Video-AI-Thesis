"""08-aggregation: collapse per-frame and per-sample outputs into per-segment records.

Aggregates at two scopes inside a segment and never across one: one record per tracked person,
keyed by (segment, track_id), and one record for the segment itself carrying the plastic timeline,
text and object extents. Track ids are segment-local, so linking people across
segments is 10-consolidation's job and must not be attempted here.

Every statistic carries its support count, and variance is null below two samples rather than
zero - with median track lengths in single digits, most supports are small and saying so is the point.
"""
...
def _pose_summary(samples: list[dict[str, Any]], floor: float) -> dict[str, Any]:
    ...
    usable: list[tuple[int, Array, Array]] = []
    for sample in samples:
        ...
        usable.append((int(sample["frame"]), joints[:, :2], keep))

    if not usable:
        return {"n": 0}

    counts = [int(keep.sum()) for _, _, keep in usable]
    spread: list[float] = []
    stacked = np.stack([xy for _, xy, _ in usable])
    masks = np.stack([keep for _, _, keep in usable])
    ...
    movement: list[float] = []
    for (_, a, ka), (_, b, kb) in itertools.pairwise(usable):
        both = ka & kb
        if both.any():
...
def _person(
    track: dict[str, Any], span: dict[str, Any], fps: float, cfg: Config
) -> dict[str, Any]:
    series = track["series"]
    ...
    record: dict[str, Any] = {
        ...
        "duration_frames": span["frame_count"],
        "duration_seconds": round(span["frame_count"] / fps, 3) if fps else 0.0,
        "support": {
            "series": len(series),
            "face": len(face),
            "pose": sum(1 for s in series if "pose" in s),
            "crops": len(attrs.get("embedding_rows", [])),
            "observed_frames": span["observed_frames"],
        },
    }
...
def run(cfg: Config) -> dict[str, Any]:
    ...
    for shot in conditional["shots"]:
        ...
        for track in shot["tracks"]:
            span = spans.get((index, int(track["track_id"])))
            if span is None:
                continue
            record = _person(track, span, fps, cfg)
    ...
    meta = {
        ...
        "top_k": cfg.agg_top,
        "note": (
            "records are scoped to one segment; the same person in two segments is two "
            "records until 10-consolidation links them"
        ),
        "shots": shots,
    }
