"""09-fusion: bind experts onto entities and compute the relations between them.

08 produced the nodes: one record per person per segment. This produces the edges,
which are the facts no single person's record can hold - who attends whom, whose affect moves together,
...
Synchrony abstains below a sample floor instead of correlating three-point series.
"""
...
def _attends(
    ...
    """Fraction of shared frames where the head points within `limit` degrees of the other
    person. Head orientation, not gaze - there is no gaze model in the pipeline."""
    hits = 0
    angles: list[float] = []
...
def _synchrony(
    ...
    """Correlation of two affect series. Below the floor it abstains: correlating a handful
    of points produces a number with no meaning, and a number invites belief."""
...
def _placement(
    ...
    """Relative position over the frames both are present. Size ratio stands in for depth
    ordering: 05 writes depth at keyframes only, so a per-frame ordering does not exist."""
    shared = sorted(set(a) & set(b))
    if not shared:
        return {"n": 0}
...
def run(cfg: Config) -> dict[str, Any]:
    ...
    aggregated = _read(cfg.out_root, "aggregated.json", "08-aggregation")
    ...
    entity_count = edge_count = attention_edges = synchrony_edges = 0
    for shot in aggregated["shots"]:
        index = int(shot["index"])
        entities: list[dict[str, Any]] = []
        for person in shot["persons"]:
            key = (index, int(person["track_id"]))
            frames = rows.get(key, {})
            # 06 clamps a face to its body box, so this validates the binding rather than
            # performing it: face and body already share one track id.
            escaped = sum(
                1
        ...
        for left, right in itertools.combinations(
            sorted(shot["persons"], key=lambda p: p["track_id"]), 2
        ):
            a_key = (index, int(left["track_id"]))
            b_key = (index, int(right["track_id"]))
            a_rows, b_rows = rows.get(a_key, {}), rows.get(b_key, {})
            together = sorted(set(a_rows) & set(b_rows))
            if not together:
                continue

            a_series, b_series = series.get(a_key, {}), series.get(b_key, {})
        ...
        # Text is bound to whichever entities its region actually overlaps. Full-width
        # captions overlap everyone, which the counts make visible rather than hide.
        present = {int(e["track_id"]) for e in entities}
        ...
        for entry in raw_text.get(index, []):
            ...
            touching = [
                ...
                if frame in rows.get((index, track_id), {})
                and _overlaps(entry["box"], rows[(index, track_id)][frame]["body"])
            ]
            text.append({**entry, "tracks": sorted(touching)})
    ...
    meta = {
        ...
        "note": (
            ...
            "floor; relations never cross a segment boundary"
        ),
        "shots": shots,
    }
